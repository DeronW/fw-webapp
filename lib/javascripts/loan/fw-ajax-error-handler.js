    ...
    function handler(code, msg, slience) {
        if (code == 0) {
            ...
            if (code == 100008) {
                ...
                $FW.Browser.inApp() ? NativeBridge.close() : ($FW.Browser.inFXHApp() ? NativeBridge.login() : login())
            }
        } else if(code == 11000){
            //11000 代表参数不完整
            !slience && $FW.Component.Alert(msg);
        }else{
            if(inHome() && !$FW.Store.getUserDict().token){
                ($FW.Browser.inApp() || $FW.Browser.inFXHApp()) ? NativeBridge.login() : login()
            }
        }
        return true;
    }

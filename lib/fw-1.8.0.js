    ...
    var fn = FW.prototype = {
        ...
        Ajax: function (options) {
            var cfg = {
                ...
                complete: null,
                fail: null,
                silence: false, // 当返回结果 code 非 10000 时, 只 reject 不做其它操作
                        ...
                        if (xhr.status == 200 || xhr.status == 201) {
                            ...
                            if (r.code == 10000) {
                                ...
                                if (cfg.fail && cfg.fail(r.code, r.message, xhr.responseText) === true) {
                                    ...
                                    // silence 参数, 表示即使出现错误也采取静默处理的方式
                                    // 采用哪种静默处理方式, 由个项目自己的异常处理函数处理
                                    FW.AjaxErrorHandler(r.code, r.message, cfg.silence);
                                    reject(r);
                                }
                            }
         ...
         * data: 请求参数
         * success: 成功后的执行函数
         * Post方法 use "silence" option,
         */
        Post: function (url, data, loading, silence) {
            return FW.Ajax({
                url: url,
                method: 'POST',
                data: data,
                enable_loading: loading && 'mini',
                silence: silence === undefined ? true : !!silence
            })
        },
        /**
         * JSONP handler

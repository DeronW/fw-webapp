...
let information =  [
  ...
  {id: 31, name: '新疆', city: [
      ...
      {id: 341, name: '图木舒克市', district: [
        ...
        {id: 2862, name: '兵团五十一团'},
        {id: 2862, name: '兵团五十三团'},
        {id: 2862, name: '兵团图木舒克市喀拉拜勒镇'},
        {id: 2862, name: '兵团图木舒克市永安坝'}
      ]},
      {id: 342, name: '五家渠市', district: [
        {id: 2856, name: '军垦路街道'},
        {id: 2857, name: '青湖路街道'},
        {id: 2858, name: '人民路街道'},
        {id: 2860, name: '101团（铁牛镇）'},
        {id: 2861, name: '102团（梧桐镇）'},
        {id: 2862, name: '103团（蔡家湖镇）'}
      ]}
  ]},
  {id: 32, name: '香港', city: [
...
export default class LoopLoanCity extends React.Component {
    state = {
        province:'',
    ...
    provinceSelected = (item,index) => {
        let cityList = [];
        information[index]['city'].forEach((provinceSelection)=>{
            cityList.push(provinceSelection.name)
        })
        this.setState({
           province:item,
           provinceSelectedIndex:index,
    ...
    citySelected = (item,index) => {
        let districtList = [];
        information[this.state.provinceSelectedIndex]['city'][index]['district'].forEach((citySelection)=>{
           districtList.push(citySelection.name)
        })
        this.setState({
          city:item,
    ...
    render() {
        ...
        let provinceItem = (item,index) => {
           ...
           return (
               <div styleName="item" key={index} onClick={handler}>{item}</div>
           )
        }
        let cityItem = (item,index) => {
          let handler = () => this.citySelected(item,index);
          return (
              <div styleName="item" key={index} onClick={handler}>{item}</div>
          )
       }
       let districtItem = (item,index) => {
          let handler = () => this.districtSelected(item,index);
          return (
              <div styleName="item" key={index} onClick={handler}>{item}</div>
          )
       }

import React from 'react'

let isActiveTab = (tab) => {
    let pt = location.pathname,hash = location.hash,cnd = false;

    if(tab == 'home' && hash == '#/')
        cnd = true;

    if(tab == 'client' && hash == '#/client')
        cnd = true;

    if(tab == 'performance' && hash == '#/performance')
        cnd = true;

    return cnd
}

let tabColor = (keyword) => {
    let cnd = isActiveTab(keyword);
    return {
        color: cnd ? "#77a4ea" : "#9b5b54"
    }
}

function getStyle(tab){
    let style = {}

    const STYLE_TAB_BASE = {
        width: "33.33%",
        display: "block",
        float: "left",
        textAlign: "center",
        fontSize: "22px",
    }
    const STYLE_ICON_BASE = {
        height: "50px",
        display: "block",
        backgroundRepeat: "no-repeat",
        backgroundPosition: "center",
        margin: "10px 0",
    }

    if (tab == 'fixed_panel'){
        style = {
            width: "720px",
            height: "94px",
            position: "fixed",
            left: "0",
            right: "0",
            bottom: "-1px",
            background: "#fff",
            zIndex: "100",
            paddingBottom:'6px'
        }
    }
    if(tab == 'tab_home'){
        style = Object.assign({}, STYLE_TAB_BASE, tabColor('home'))
    }
    if(tab == 'tab_client'){
        style = Object.assign({}, STYLE_TAB_BASE, tabColor('client'))
    }
    if(tab == 'tab_performation'){
        style = Object.assign({}, STYLE_TAB_BASE, tabColor('performation'))
    }

    if(tab == 'icon_home'){
        style = Object.assign({},STYLE_ICON_BASE,{
            backgroundImage:isActiveTab("home")?
                            "url('data:image/png;base64,')":
                            "url('data:image/png;base64,')"

        })
    }
    if(tab == 'icon_client'){
        style = Object.assign({},STYLE_ICON_BASE,{
            backgroundImage:isActiveTab("client")?
                            "url('data:image/png;base64,')":
                            "url('data:image/png;base64,')"
        })
    }
    if(tab == 'icon_performation'){
        style = Object.assign({},STYLE_ICON_BASE,{
            backgroundImage:isActiveTab("performation")?
                            "url('data:image/png;base64,')":
                            "url('data:image/png;base64,')"
        })
    }

    return style
}

class BottomNavBar extends React.Component{
    render(){
        // let { history } = this.props
        let link_handler = tab => {
            if (isActiveTab(tab)) return;

            if(tab == 'home'){
                location.href = '/static/manager/jrgc/index.html#/'
            }
            if(tab == 'client'){
                location.href = '/static/manager/jrgc/index.html#/client'
            }
            if(tab == 'performation'){
                location.href = '/static/manager/jrgc/index.html#/performation'
            }
        }
        return <div>
            <div style={getStyle('fixed_panel')}>
                <a style={getStyle('tab_home')} onClick={() => link_handler('home')}>
                    <i style={getStyle('icon_home')}></i>我的
                </a>
                <a style={getStyle('tab_client')} onClick={() => link_handler('client')}>
                    <i style={getStyle('icon_client')}></i>客户
                </a>
                <a style={getStyle('tab_performation')} onClick={() => link_handler('performation')}>
                    <i style={getStyle('icon_performation')}></i>业绩
                </a>
            </div>
        </div>
    }
}

export default BottomNavBar